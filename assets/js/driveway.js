$(document).ready(function() { 
    ...
    if(goclick == 1){
        $(".gobtn").removeClass('btn-brand');
        $(".gobtn").addClass('btn-go pulse animated');	
    }    
    });
    ...
    $('input[name=time1]').change(function() {
    if(goclick == 1){
        $(".gobtn").removeClass('btn-brand');
        $(".gobtn").addClass('btn-go pulse animated');	
    }    
    });
    $('input[name=time2]').change(function() {
    if(goclick == 1){
        $(".gobtn").removeClass('btn-brand');
        $(".gobtn").addClass('btn-go pulse animated');	
    }    
    });
    $('input:radio[name=bookingoption]').change(function(){
      if(goclick == 1){
        $(".gobtn").removeClass('btn-brand');
        $(".gobtn").addClass('btn-go pulse animated');	
    }          
    ...
    var search_lngG;
    var goclick = 0;
    var stripeChargeFeeC = 0;
    var stripeProcessFeeC = 0;
    var applicationFeeC = 0; 
    var applicationFeescents = 0;
   // var totalDays = 0;
    processCharge();
    function processCharge(){
        $.ajax({
                            method : "POST",
                            url : baseUrl+ "driveway/processCharge/",						
                            dataType : "json",
                            success : function(response) {
                                  stripeChargeFeeC = response.stripefee;
                                  stripeProcessFeeC = response.stripefee2;
                                  applicationFeeC = response.applicationfee;
                                //  totalDays = response.totaldays;
                                  applicationFeescents = response.applicationfee2;
    ...
    if (navigator.geolocation)
    {
        ...
        map.addListener('bounds_changed', function() {cardType
	...
	// function for filtering date, location and price
	    ...
	    if ($('#timecheck').val() == ''){	
            ...
            goclick   = 1;
            $(".gobtn").removeClass('btn-go pulse animated');
            $(".gobtn").addClass('btn-brand');
        ...
        myFunction = function(id) {       
            ...
            $.ajax({
                ...
                success : function(response) {				
						...
						{
						if (hourChargeG[id]){
							totalPrice  = priceG*hourChargeG[id];
                                                    } else {
                                                       totalPrice  = priceG * $('#hchargeset').val(); 
                                                    }
						}
						else
						{
                                                    if (hourChargeG[id]){
							hoursplit		= hourChargeG[id].split(":");
                                                    } else {
                                                        var hcharge = $('#hchargeset').val(); 
                                                        hoursplit		= hcharge.split(":");
                                                    }
							hourprice       = hoursplit[0]*priceG;
							minuteprice     = 0;
							if(hoursplit[1]>0)
							{
								minuteprice     = hoursplit[1]*priceG/60;
							}
                        ...
                        strDate = hourChargeG[drivewayIdG]; 
                        } else {
                            strDate = $('#hchargeset').val();
                        }
                        if(option == 1){											
                            $("label[for='message']").html('You Have selected Hourly Booking Type. '+strDate+' Hours rental fee is <span> $'+ totalPriceG+' </span>');
                        }else if(option == 2){
                            $("label[for='message']").html('You Have selected Recurring Booking Type. Total '+strDate+' Hours rental fee is <span> $'+ totalPriceG+'</span>.<br/> Everyday you can Check-In after '+ response['bookingFromtime']+' and Check-Out before '+ response['bookingTotime']);
                        }else if(option == 3){
                            $("label[for='message']").html('You Have selected Flat rate Booking Type. '+strDate+' Days rental fee is <span> $'+ totalPriceG+' </span>');
                        }		
                        ...
                        $("p[for='username']").html(response['driveway'].firstname+'&nbsp'+response['driveway'].lastname);
    ...
    $(".rentDriveway").on("click",function() 
    {
        ...
        $.ajax({
            ...
            success  : function(data) {													
                if (data["login"][0]["status"]) {
                    ...
                    proc_feeG         = ((parseFloat(stripeChargeFeeC) / 100) * hourlyPrice) + parseFloat(stripeProcessFeeC)+ parseFloat(applicationFeeC); 
                    proc_feeG        =  proc_feeG.toFixed(2);         
                    var grandTotal    =   parseFloat(hourlyPrice) + parseFloat(proc_feeG);
                    grandTotal        = grandTotal.toFixed(2);  */
                    
                    var strDate2 = 0;
                        strDate2 = hourChargeG[drivewayIdG]; 
                    ...
                    if (hourChargeG[drivewayIdG]){ 
						...
						if(optionG==3)
						{
							...
							totalPrice2  = priceG*hourChargeG[drivewayIdG];
						}
						else
						{
							hoursplit		= hourChargeG[drivewayIdG].split(":");
							hourprice       = hoursplit[0]*priceG;
							minuteprice     = 0;
							if(hoursplit[1]>0)
							{
							...
							totalPrice2  = hourprice+minuteprice;
						}						
                        totalPriceG2     = totalPrice2.toFixed(2);                        
                       /* var totalHrPrice  = priceG*hourChargeG[drivewayIdG];	
                    hourlyPrice       = totalHrPrice.toFixed(2); */
                    hourlyPrice = totalPriceG2;
                    proc_fee         = ((parseFloat(stripeChargeFeeC) / 100) * totalPriceG2) + parseFloat(stripeProcessFeeC)+ parseFloat(applicationFeeC); 
                    proc_fee         = proc_fee.toFixed(2);         
                    var grandTotal   = parseFloat(totalPriceG2) + parseFloat(proc_fee);
                    grandTotal       = grandTotal.toFixed(2); 
                    proc_feeG        = ((parseFloat(stripeChargeFeeC) / 100) * grandTotal) + parseFloat(stripeProcessFeeC)+ parseFloat(applicationFeeC); 
                    proc_feeG        = proc_feeG.toFixed(2);
                    var grandTotalG  = parseFloat(totalPriceG2) + parseFloat(proc_feeG);
                    grandTotalG      = grandTotalG.toFixed(2); 
                        if(optionG == 3){
                            $('.driveway-price').html('<p> <div>Total '+hourChargeG[drivewayIdG]+' Days rental fee: $'+totalPriceG2+'</div> <div> Processing Fee: $'+proc_feeG+'</div><div>Total Charge : <span>'+grandTotalG+' </span></div> </p>'); 
                        }
                        else
                        {
                            $('.driveway-price').html('<p> <div>Total '+strDate2+' Hours rental fee: $'+ totalPriceG2 +' </div> <div> Processing Fee: $'+proc_feeG+'</div><div>Total Charge : <span> $'+grandTotalG+'</span></div> </p>');
                        }
                    }														
		...
		{          var optionG        = searchTypeG[drivewayIdG];	
                        ...
                        hourlyPriceGl     = totalPrice2.toFixed(2);
	...
	if(!$('#veicleList').val()) {
            ...
            if (model == ''){
                ...
                $('#error_msg_v').html('Enter License Plate Number');
                return;
            }
            ...
            {						
                ...
                $.ajax({
                        ...
                        success  : function(response) {                         
                            ...
                            } else  if(response.status == false && response.message == 'session timeout') {
                                $('.wait').hide();
    ...
    {						
        ...
        var mapf        = new google.maps.Map(document.getElementById('driveway-map_final'), {
          zoom  : 8,
          center: myLatLng
        });
