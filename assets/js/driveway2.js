        ...
        myFunction = function(id) {       
            ...
            $.ajax({
                ...
                success : function(response) {				
                        ...
                        hour = $.trim(arr[0]);
                        min = $.trim(arr[1]);
                        var d = moment.duration({hours: hour, minutes: min});                        
                        if(d.hours()==0){
                        var as_time = hourChargeG[drivewayIdG];                        
                        }
                        else{
                        var as_time = d.hours() + ":" + d.minutes();                         
                        }  
                      
                        if(option == 1){											
                            $("label[for='message']").html('You Have selected Hourly Booking Type. '+as_time+' Hours rental fee is <span> $'+ totalPriceG+' </span>');
                        }else if(option == 2){
                            $("label[for='message']").html('You Have selected Recurring Booking Type. Total '+as_time+' Hours rental fee is <span> $'+ totalPriceG+'</span>.<br/> Everyday you can Check-In after '+ response['bookingFromtime']+' and Check-Out before '+ response['bookingTotime']);
                        }else if(option == 3){
            ...
            success  : function(data) {													
                if (data["login"][0]["status"]) {
                    ...
                    grandTotal        = grandTotal.toFixed(2);  
                     ...
                     var strHour = 0;
                        strHour = hourChargeG[drivewayIdG];                        
                        arr = strHour.toString().split('.');
                        hour = $.trim(arr[0]);
                        min = $.trim(arr[1]);
                        var d = moment.duration({hours: hour, minutes: min});                        
                        if(d.hours()==0){
                        var astime = hourChargeG[drivewayIdG];                        
                        }
                        else{
                        var astime = d.hours() + ":" + d.minutes();                         
                        }  
                    ...
                    if (hourChargeG[drivewayIdG]){		
                        ...
                        {
                            $('.driveway-price').html('<p> <div>Total '+astime+' Hours rental fee: $'+ hourlyPrice +' </div> <div> Processing Fee: $'+proc_feeG+'</div><div>Total Charge : <span> $'+grandTotal+'</span></div> </p>');
                        }
                    }														
