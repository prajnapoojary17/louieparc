...
;(function () {
	...
	var rootwizard = function() {
	  $('#rootwizard').bootstrapWizard({'nextSelector': '#fb-signup, .wizard .next, .btn-press, .gonext','previousSelector': '.goprevious', onTabShow: function(tab, navigation, index) {
	...
	var timePicker = function() {
		...
		// Time picker
	var dtimePicker = function() {
		$('.dtimepicker-no-seconds').timepicker({
			autoclose: true,
			minuteStep: 1,
			defaultTime: null
		});
		...
		// handle input group button click
		$('.dtimepicker').parent('.input-group').on('click', '.input-group-btn', function(e){
			e.preventDefault();
			defaultTime: '10:45 PM'
			$(this).parent('.input-group').find('.dtimepicker').timepicker('showWidget');
		});
	};
	...
	var inputmask = function() {
	  ...
	  $("#currency1").inputmask({ alias: "currency"});
	};
