        ...
        $.ajax({
            ...
            success  : function(data) {													
                ...
                if(optionG==3)
                       ...
                       /* var totalHrPrice  = priceG*hourChargeG[drivewayIdG];	
                    hourlyPrice       = totalHrPrice.toFixed(2); */
                    proc_feeG         = ((parseFloat(stripeChargeFeeC) / 100) * totalPriceG2) + parseFloat(stripeProcessFeeC)+ parseFloat(applicationFeeC); 
                    proc_feeG        =  proc_feeG.toFixed(2);         
                    var grandTotal    =   parseFloat(totalPriceG2) + parseFloat(proc_feeG);
                    grandTotal        = grandTotal.toFixed(2); 
                    
                        if(optionG == 3){
                            $('.driveway-price').html('<p> <div>Total '+hourChargeG[drivewayIdG]+' Days rental fee: $'+totalPriceG2+'</div> <div> Processing Fee: $'+proc_feeG+'</div><div>Total Charge : <span>'+grandTotal+' </span></div> </p>'); 
                        }
                        else
                        {
                            $('.driveway-price').html('<p> <div>Total '+strDate2+' Hours rental fee: $'+ totalPriceG2 +' </div> <div> Processing Fee: $'+proc_feeG+'</div><div>Total Charge : <span> $'+grandTotal+'</span></div> </p>');
                        }
                    }														
